...
void Task::setAssignedTo(const std::string &userId) { assignedTo = userId; }
void Task::setStatus(TaskStatus status) { this->status = status; }

void Task::setPriority(TaskPriority priority) { this->priority = priority; }
void Task::setDueDate(std::time_t dueDate) { this->dueDate = dueDate; }

void Task::addDependency(const std::string &taskId) {
  if (std::find(dependencies.begin(), dependencies.end(), taskId) ==
      dependencies.end()) {
...
void Task::displayInfo() const {
  ...
  std::cout << "Assigned to: "
            << (assignedTo.empty() ? "Unassigned" : assignedTo) << std::endl;
