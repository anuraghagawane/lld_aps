...
User *TaskManager::registerUser(const std::string &username,
...
void TaskManager::removeTask(const std::string &taskId) {
  auto it = std::find_if(tasks.begin(), tasks.end(), [taskId](Task *task) {
...
bool TaskManager::assignTask(const std::string &taskId,
...
bool TaskManager::updateTaskStatus(const std::string &taskId,
                                   TaskStatus status) {
  Task *task = findTask(taskId);
  if (!task)
    return false;
...
bool TaskManager::addTaskDependency(const std::string &taskId,
...
bool TaskManager::addTaskComment(const std::string &taskId,
...
void TaskManager::displayUserTasks(const std::string &userId) const {
  ...
  std::cout << "\nTasks assigned to " << user->getUsername() << ":"
            << std::endl;
...
std::vector<Task *>
TaskManager::getTasksByPriority(TaskPriority priority) const {
  std::vector<Task *> result;
  std::copy_if(
