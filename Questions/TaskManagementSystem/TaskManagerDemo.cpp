...
int main() {
  ...
  Task *task1 = manager.createTask("Design Database", "Create database schema",
                                   TaskPriority::HIGH);
  Task *task2 = manager.createTask(
      "Implement API", "Develop REST API endpoints", TaskPriority::MEDIUM);
  Task *task3 = manager.createTask("Write Tests", "Create unit tests",
                                   TaskPriority::MEDIUM);

  manager.addTaskDependency(task2->getTaskId(),
                            task1->getTaskId()); // API depends on DB
  manager.addTaskDependency(task3->getTaskId(),
                            task2->getTaskId()); // Tests depend on API
  //
  ...
  manager.addTaskComment(task1->getTaskId(),
                         "Started working on schema design");
