...
StackOverflow::StackOverflow()
    : userIdCounter(1), postIdCounter(1), commentIdCounter(1) {}
...
User *StackOverflow::registerUser(const std::string &username,
...
Post *StackOverflow::addQuestion(const std::string &userId,
...
Post *StackOverflow::addAnswer(const std::string &userId,
  ...
  std::string postId = generatePostId();
  Post *answer = new Post(postId, userId, content, std::vector<std::string>(),
                          PostType::ANSWER);
  posts.push_back(answer);
  return answer;
}

Comment *StackOverflow::addComment(const std::string &userId,
...
bool StackOverflow::votePost(const std::string &userId,
                             const std::string &postId) {
...
bool StackOverflow::unvotePost(const std::string &userId,
                               const std::string &postId) {
  Post *post = findPost(postId);
  if (!post)
    return false;
...
bool StackOverflow::acceptAnswer(const std::string &userId,
...
std::vector<Post *>
StackOverflow::searchQuestions(const std::string &tag) const {
...
void StackOverflow::updateUserReputation(const std::string &userId,
                                         int points) {
  if (User *user = findUser(userId)) {
    user->udpateReputation(points);
  }
}
