...
User::User(std::string userId, std::string username, std::string email)
    : userId(userId), username(username), email(email), reputation(1),
      active(true) {}
...
void User::udpateReputation(int points) { reputation += points; }

void User::addBadges(const std::string &badge) { badges.push_back(badge); }
