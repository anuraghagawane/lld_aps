...
Post::Post(std::string postId, std::string userId, std::string content,
    ...
    : postId(postId), userId(userId), content(content), tags(tags), score(0),
      accepted(false), type(type) {
...
void Post::addComment(Comment *comment) { comments.push_back(comment); }
...
void Post::setAccepted(bool status) { accepted = status; }

void Post::displayInfo() const {
  std::cout << "\nPost ID: " << postId << std::endl;
  std::cout << "Type: " << (type == PostType::QUESTION ? "Question" : "Answer")
            << std::endl;
  std::cout << "Content: " << content << std::endl;
  std::cout << "Score: " << score << std::endl;
  std::cout << "Status: " << (accepted ? "Accepted" : "Not Accepted")
            << std::endl;
