...
public:
  ...
  double getCost() override { return inner->getCost() + 0.50; }
...
public:
  ...
  double getCost() override { return inner->getCost() + 0.20; }
...
public:
  ...
  double getCost() override { return inner->getCost() + 1.00; }
...
int main() {
  SimpleCoffee simple;
  printf("Order 1: %s | $%.2f\n", simple.getDescription().c_str(),
         simple.getCost());

  MilkDecorator milk(&simple);
  SugarDecorator milkSugar(&milk);
  printf("Order 2: %s | $%.2f\n", milkSugar.getDescription().c_str(),
         milkSugar.getCost());
